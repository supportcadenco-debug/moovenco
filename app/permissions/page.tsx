...
const ACCESS_LEVELS = [
  ...
  { value: 'add',  label: '➕ Ajout seul',  color: '#FFF3E0', text: '#D4720A' },
  { value: 'own',  label: '🔒 Ses données', color: '#F3E5F5', text: '#7B1FA2' },
  { value: 'full', label: '✅ Complet',     color: '#E8F5E9', text: '#1A9E50' },
]
...
export default function Permissions() {
  ...
  async function loadPermissions() {
    const { data, error } = await supabase
      .from('permissions').select('*').eq('company_id', COMPANY_ID)
    if (!error && data) {
      const map = {}
      data.forEach(p => {
  ...
  function getAccess(role, module) {
    return permissions[role]?.[module] || 'none'
  }

  async function setAccess(role, module, access) {
    setPermissions(prev => ({ ...prev, [role]: { ...(prev[role] || {}), [module]: access } }))
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', fontFamily: 'Inter, sans-serif', background: '#ECEEF1' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ width: '220px', minWidth: '220px', background: 'white', borderRight: '1px solid #D0D4DA', overflow: 'auto' }}>
          <div style={{ padding: '12px 14px', fontSize: '10px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '.5px', color: '#8A95A3', borderBottom: '1px solid #F0F2F5' }}>
            Rôles
          </div>
          {ROLES.map(role => (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: '#F8F9FB', borderBottom: '1px solid #E2E6EA' }}>
                      ...
                      <th style={{ padding: '12px 16px', fontSize: '10px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '.5px', color: '#8A95A3', textAlign: 'left' }}>Description</th>
                    </tr>
                  </thead>
                  <tbody>
                    {MODULES.map((mod, i) => {
                            ...
                            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                              {ACCESS_LEVELS.map(level => (
                                <button key={level.value} onClick={() => setAccess(selectedRole, mod.value, level.value)}
                                  style={{ background: current === level.value ? level.color : '#F8F9FB', color: current === level.value ? level.text : '#8A95A3', border: `1px solid ${current === level.value ? level.text + '40' : '#E2E6EA'}`, fontFamily: 'inherit', fontSize: '10px', fontWeight: current === level.value ? '700' : '500', padding: '4px 10px', borderRadius: '12px', cursor: 'pointer' }}>
                                  {level.label}
                                </button>
                              ))}
                            </div>
                          </td>
                          <td style={{ padding: '14px 16px', fontSize: '11px', color: '#8A95A3' }}>
                            {current === 'none' && 'Ce rôle ne voit pas ce module'}
                            {current === 'read' && 'Peut consulter mais pas modifier'}
                            {current === 'add' && 'Peut ajouter ses propres entrées uniquement'}
                            {current === 'own' && 'Accès limité à ses propres données'}
                            {current === 'full' && 'Accès complet — lecture, création, modification'}
                          </td>
                        </tr>
                      )
                    })}
              ...
              <div style={{ marginTop: '20px', background: 'white', borderRadius: '10px', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,.06)' }}>
                ...
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '700px' }}>
                    <thead>
                      <tr style={{ background: '#F8F9FB', borderBottom: '1px solid #E2E6EA' }}>
                        <th style={{ padding: '8px 12px', fontSize: '10px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '.5px', color: '#8A95A3', textAlign: 'left', width: '120px' }}>Module</th>
                        {ROLES.map(r => (
                          ...
                          {ROLES.map(role => {
                            ...
                            return (
                              <td key={role.value} style={{ padding: '8px 10px', textAlign: 'center' }}>
                                <span style={{ background: level?.color, color: level?.text, fontSize: '9px', fontWeight: '700', padding: '2px 6px', borderRadius: '8px', whiteSpace: 'nowrap' }}>
                                  {access === 'none' ? '—' : level?.label.split(' ')[0]}
                                </span>
                              </td>
                            )
                          })}
