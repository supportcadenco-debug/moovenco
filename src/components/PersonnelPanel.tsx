...
function ExpiryTag({ date, label }: any) {
...
function CircuitAssigner({ circuits, driverCircuits, onAdd, saving }: any) {
  const [sel, setSel] = useState('')
  const [jours, setJours] = useState([...JOURS])
  const toggle = (j: string) => setJours((p: any) => p.includes(j) ? p.filter((x: any) => x !== j) : [...p, j])
  const available = circuits.filter((c: any) => !driverCircuits.find((dc: any) => dc.circuit_id === c.id))
  return (
    <div style={{ background:'#F8F9FB', borderRadius:'8px', padding:'10px 12px' }}>
      ...
      <select value={sel} onChange={e => setSel(e.target.value)}
        ...
        {available.map((c: any) => <option key={c.id} value={c.id}>{c.code ? `[${c.code}] ` : ''}{c.name}</option>)}
      </select>
...
/**
 * Composant partagé — logique métier et JSX du module Personnel.
 * Ne contient AUCUN contrôle RBAC (pas de useAuth) : le point d'entrée
 * appelant (app/comptabilite/personnel.tsx en mode embarqué, ou
 * app/personnel/page.tsx en mode standalone) est responsable de vérifier
 * l'accès au module `personnel` et de ne monter ce composant qu'une fois
 * l'utilisateur autorisé. Cela évite une double redirection plein écran
 * lorsque ce composant est rendu à l'intérieur de l'onglet Comptabilité.
 */
export default function PersonnelPanel({ currentUserProfile }: { currentUserProfile?: any }) {
  ...
  const [mainView, setMainView]          = useState<'liste' | 'absences'>('liste')
  ...
  const [driverCircuits, setDriverCircuits] = useState<any[]>([])
  ...
  async function loadDriverCircuits(driverId: string) {
  ...
  async function traiterAbsence(id: string, statut: 'approved' | 'refused') {
    ...
    await supabase.from('absences').update({
      statut,
      traite_par: currentUserProfile?.name || 'Exploitant',
  ...
  async function addDriverCircuit(circuitId: string, jours: string[]) {
    if (!selected) return
    setSavingCircuit(true)
    const existing = driverCircuits.find((dc: any) => dc.circuit_id === circuitId)
    if (!existing) {
  ...
  async function removeDriverCircuit(id: string) {
    await supabase.from('driver_circuits').delete().eq('id', id)
    setDriverCircuits((prev: any) => prev.filter((dc: any) => dc.id !== id))
  }

  async function uploadDoc(e: any, categorie: string) {
  ...
  async function deleteDoc(doc: any) {
    ...
    setStaffDocs((prev: any) => prev.filter((d: any) => d.id !== doc.id))
  }
  ...
  function handleNameChange(e: any) {
    ...
    setForm((f: any) => ({ ...f, name, initials: name.split(' ').map((w: string) => w[0]).join('').toUpperCase().slice(0,2) }))
  }

  function isDriverRole(person: any) {
  ...
  async function handleSaveDriverDetails() {
    ...
    const payload = {
      id: selected.id, company_id: COMPANY_ID,
    ...
    if (error) setMessage('Erreur : ' + error.message)
    else { setMessage('✅ Fiche enregistrée'); setEditingDriver(false); loadAll() }
    setSaving(false)
  }
  ...
  function openSelected(person: any) {
    setSelected(person)
    setShowForm(false)
  ...
  function togglePermis(p: string) {
    setDriverForm((f: any) => ({ ...f, permis: f.permis.includes(p) ? f.permis.filter((x: string) => x !== p) : [...f.permis, p] }))
  }
  ...
  const inputSt: any = { width:'100%', padding:'7px 9px', border:'1px solid #D0D4DA', borderRadius:'5px', fontSize:'11px', fontFamily:'inherit', boxSizing:'border-box' }
  const allCats = [...DOCS_COMMUNS, ...(selected && isDriverRole(selected) ? DOCS_CONDUCTEUR : [])]
  const pendingCount = allAbsences.filter((a: any) => a.statut === 'pending').length
  const displayedAbsences = absenceFilter === 'pending' ? allAbsences.filter((a: any) => a.statut === 'pending') : allAbsences
    ...
    <div style={{ display:'flex', flexDirection:'column', flex: 1, minHeight: 0, fontFamily:'Inter, sans-serif' }}>
      ...
      <div style={{ background:'#253044', padding:'0 16px', height:'38px', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
        <div style={{ display:'flex', gap:'6px', alignItems:'center' }}>
          ...
          {mainView === 'absences' && (
            <span style={{ fontSize:'12px', fontWeight:'700', color:'white' }}>🧳 Demandes d'absence</span>
          )}
        </div>
        <div style={{ display:'flex', gap:'8px', alignItems:'center' }}>
          {/* Toggle vue absences */}
          <button onClick={() => { setMainView(mainView === 'absences' ? 'liste' : 'absences'); setSelected(null); setShowForm(false) }}
            style={{ position:'relative', background: mainView==='absences' ? '#2EC971' : 'rgba(255,255,255,.15)', border:'none', color:'white', fontFamily:'inherit', fontSize:'11px', fontWeight:'700', padding:'4px 12px', borderRadius:'5px', cursor:'pointer', display:'flex', alignItems:'center', gap:'6px' }}>
            🧳 Absences
            {pendingCount > 0 && (
              <span style={{ background:'#C62828', color:'white', fontSize:'9px', fontWeight:'700', borderRadius:'20px', padding:'1px 5px', minWidth:'16px', textAlign:'center' }}>
                {pendingCount}
              </span>
            )}
          </button>
          <button onClick={() => { setShowForm(true); setSelected(null); setMainView('liste') }}
      ...
      {mainView === 'absences' && (
        <div style={{ flex:1, overflow:'auto', padding:'14px' }}>
          <div style={{ background:'white', borderRadius:'10px', boxShadow:'0 1px 3px rgba(0,0,0,.06)', overflow:'hidden' }}>
            {/* Filtres */}
            <div style={{ padding:'10px 16px', borderBottom:'1px solid #E2E6EA', display:'flex', gap:'8px', alignItems:'center' }}>
              {[['pending','⏳ En attente'],['all','Toutes']].map(([k,l]) => (
                <button key={k} onClick={() => setAbsenceFilter(k as any)}
                  style={{ background: absenceFilter===k ? '#1A2130' : '#F8F9FB', color: absenceFilter===k ? 'white' : '#4A5568', border:'1px solid #D0D4DA', fontFamily:'inherit', fontSize:'10px', fontWeight:'600', padding:'3px 12px', borderRadius:'10px', cursor:'pointer' }}>
                  {l} {k === 'pending' ? `(${pendingCount})` : `(${allAbsences.length})`}
                </button>
              ))}
            </div>
            {/* Tableau */}
            {displayedAbsences.length === 0 ? (
              <div style={{ textAlign:'center', color:'#8A95A3', padding:'40px', fontSize:'13px' }}>
                {absenceFilter === 'pending' ? '✅ Aucune demande en attente' : 'Aucune demande d\'absence'}
              </div>
            ) : (
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'11px' }}>
                <thead>
                  <tr style={{ background:'#F8F9FB', borderBottom:'1px solid #E2E6EA' }}>
                    {['Conducteur','Type','Période','Commentaire','Statut','Actions'].map((h,i) => (
                      <th key={h} style={{ padding:'9px 14px', fontSize:'10px', fontWeight:'700', textTransform:'uppercase', letterSpacing:'.5px', color:'#8A95A3', textAlign: i===0?'left':'center' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {displayedAbsences.map((a: any, i: number) => {
                    const st = STATUT_ABS[a.statut] || STATUT_ABS.pending
                    const conducteur = staff.find((s: any) => s.id === a.conducteur_uid)
                    return (
                      <tr key={a.id} style={{ borderBottom:'1px solid #F0F2F5', background: i%2===0?'white':'#FAFBFC' }}>
                        <td style={{ padding:'10px 14px' }}>
                          <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                            {conducteur && (
                              <div style={{ width:'28px', height:'28px', borderRadius:'50%', background:conducteur.color, color:'white', fontSize:'10px', fontWeight:'700', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                                {conducteur.initials}
                              </div>
                            )}
                            <span style={{ fontWeight:'600', color:'#1A2130' }}>{a.conducteur_nom || conducteur?.name || '—'}</span>
                          </div>
                        </td>
                        <td style={{ padding:'10px 14px', textAlign:'center', color:'#1A2130' }}>{a.type}</td>
                        <td style={{ padding:'10px 14px', textAlign:'center', color:'#4A5568', whiteSpace:'nowrap' }}>
                          {new Date(a.date_debut).toLocaleDateString('fr-FR')} → {new Date(a.date_fin).toLocaleDateString('fr-FR')}
                        </td>
                        <td style={{ padding:'10px 14px', textAlign:'center', color:'#8A95A3', maxWidth:'140px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                          {a.commentaire || '—'}
                        </td>
                        <td style={{ padding:'10px 14px', textAlign:'center' }}>
                          <span style={{ background: st.bg, color: st.color, fontSize:'10px', fontWeight:'700', padding:'3px 10px', borderRadius:'10px' }}>{st.label}</span>
                        </td>
                        <td style={{ padding:'10px 14px', textAlign:'center' }}>
                          {a.statut === 'pending' ? (
                            <div style={{ display:'flex', gap:'5px', justifyContent:'center' }}>
                              <button onClick={() => traiterAbsence(a.id, 'approved')} disabled={savingAbsence}
                                style={{ background:'#E8F5E9', border:'1px solid #A5D6A7', color:'#1A9E50', fontFamily:'inherit', fontSize:'10px', fontWeight:'700', padding:'4px 10px', borderRadius:'5px', cursor:'pointer' }}>
                                ✓ Approuver
                              </button>
                              <button onClick={() => traiterAbsence(a.id, 'refused')} disabled={savingAbsence}
                                style={{ background:'#FFEBEE', border:'1px solid #FFCDD2', color:'#C62828', fontFamily:'inherit', fontSize:'10px', fontWeight:'700', padding:'4px 10px', borderRadius:'5px', cursor:'pointer' }}>
                                ✕ Refuser
                              </button>
                            </div>
                          ) : (
                            <span style={{ fontSize:'10px', color:'#8A95A3' }}>{a.traite_par ? `par ${a.traite_par}` : '—'}</span>
                          )}
                        </td>
                      </tr>
                    )
                    ...
                    {filtered.map((person: any, i: number) => {
                      ...
                      return (
                        <tr key={person.id} style={{ borderBottom:'1px solid #F0F2F5', background: i%2===0?'white':'#FAFBFC', cursor:'pointer' }}
                            ...
                            <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                              ...
                              <div>
                                <div style={{ fontSize:'12px', fontWeight:'600', color:'#1A2130', display:'flex', alignItems:'center', gap:'6px' }}>
                                  {person.name}
                                  {personPending > 0 && <span style={{ background:'#FFF3E0', color:'#D4720A', fontSize:'9px', fontWeight:'700', padding:'1px 5px', borderRadius:'8px' }}>🧳 {personPending}</span>}
                                </div>
                          ...
                          <td style={{ padding:'10px 14px' }}>
                            {pd?.permis_categories ? (
                              <div style={{ display:'flex', gap:'3px', flexWrap:'wrap' }}>
                                {pd.permis_categories.split(',').filter(Boolean).map((p: string) => <span key={p} style={{ background:'#1A2130', color:'white', fontSize:'9px', fontWeight:'700', padding:'2px 5px', borderRadius:'4px' }}>{p.trim()}</span>)}
                              </div>
                          ...
                          <td style={{ padding:'10px 14px' }} onClick={(e: any) => e.stopPropagation()}>
            ...
            <div style={{ width:'360px', minWidth:'360px', background:'white', borderLeft:'1px solid #D0D4DA', display:'flex', flexDirection:'column', overflow:'hidden' }}>
              ...
              {selected && !showForm && (
                <div style={{ display:'flex', borderBottom:'1px solid #E2E6EA', background:'white', flexShrink:0, overflowX:'auto' }}>
                  {[
                    ...
                    const absPending = k === 'absences' ? absences.filter((a: any) => a.statut === 'pending').length : 0
                    return (
                      <button key={k} onClick={() => setTab(k)}
                        style={{ flex:1, background:'none', border:'none', borderBottom: tab===k?'2px solid #0E5AA7':'2px solid transparent', color: tab===k?'#0E5AA7':'#8A95A3', fontFamily:'inherit', fontSize:'10px', fontWeight:'600', padding:'8px 4px', cursor:'pointer', whiteSpace:'nowrap', display:'flex', alignItems:'center', justifyContent:'center', gap:'3px' }}>
                        {l}
                        {absPending > 0 && <span style={{ background:'#C62828', color:'white', fontSize:'8px', fontWeight:'700', borderRadius:'20px', padding:'1px 4px' }}>{absPending}</span>}
                      </button>
                    )
                  })}
                </div>
              ...
              <div style={{ flex:1, overflow:'auto', padding:'14px 16px' }}>
                ...
                {showForm && (
                  <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
                    ...
                    <div>
                      ...
                      <input value={form.initials} onChange={e => setForm((f: any) => ({ ...f, initials: e.target.value.toUpperCase().slice(0,2) }))} maxLength={2}
                    ...
                    <div>
                      ...
                      <div style={{ display:'flex', gap:'6px', flexWrap:'wrap' }}>
                        {COLORS.map(c => (
                          <div key={c} onClick={() => setForm((f: any) => ({ ...f, color: c }))}
                    ...
                    <div>
                      ...
                      <select value={form.role} onChange={e => setForm((f: any) => ({ ...f, role: e.target.value }))} style={{ ...inputSt }}>
                    ...
                    {form.role !== 'conducteur' && (
                      <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'8px 10px', background:'#F0F7FF', borderRadius:'6px' }}>
                        <input type="checkbox" id="cond-sec" checked={form.is_conducteur_secondaire}
                          onChange={e => setForm((f: any) => ({ ...f, is_conducteur_secondaire: e.target.checked }))} />
                    ...
                    <div>
                      ...
                      <select value={form.contract} onChange={e => setForm((f: any) => ({ ...f, contract: e.target.value }))} style={{ ...inputSt }}>
                        <option value="">— Sélectionner —</option>
                    ...
                    {(form.role === 'conducteur' || form.is_conducteur_secondaire) && (
                      <div style={{ background:'#F0F7FF', border:'1px solid #BBDEFB', borderRadius:'8px', padding:'10px 12px', display:'flex', flexDirection:'column', gap:'8px' }}>
                        ...
                        <div>
                          ...
                          <input type="email" value={form.email} onChange={e => setForm((f: any) => ({ ...f, email: e.target.value }))} placeholder="jean.dupont@rgom.fr" style={{ ...inputSt }} />
                        </div>
                        <div>
                          <label style={{ fontSize:'10px', fontWeight:'600', color:'#4A5568', display:'block', marginBottom:'4px' }}>Mot de passe provisoire *</label>
                          <input type="password" value={form.password} onChange={e => setForm((f: any) => ({ ...f, password: e.target.value }))} placeholder="••••••••" style={{ ...inputSt }} />
                        </div>
                      </div>
                    )}
                        ...
                        {editingDriver ? (
                          <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
                            ...
                            <div style={{ display:'flex', alignItems:'center', gap:'8px', padding:'8px 10px', background:'#F8F9FB', borderRadius:'6px' }}>
                              <input type="checkbox" id="dispo-vac" checked={driverForm.dispo_vacances} onChange={e => setDriverForm((f: any) => ({ ...f, dispo_vacances: e.target.checked }))} />
                          ...
                          <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
                            {dd.permis_categories && (
                              <div style={{ display:'flex', gap:'4px', flexWrap:'wrap', marginBottom:'4px' }}>
                                {dd.permis_categories.split(',').filter(Boolean).map((p: string) => (
                ...
                {selected && !showForm && tab === 'documents' && (
                  <div>
                    {allCats.map((cat: any) => {
                      const catDocs = staffDocs.filter((d: any) => d.categorie === cat.key)
                      return (
                        <div key={cat.key} style={{ marginBottom:'16px' }}>
                          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'6px' }}>
                            ...
                            <label style={{ background: uploadingDoc?'#8A95A3':'#E8F0FB', color:'#0E5AA7', fontSize:'10px', fontWeight:'600', padding:'3px 8px', borderRadius:'4px', cursor: uploadingDoc?'not-allowed':'pointer' }}>
                              ...
                              <input type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display:'none' }} disabled={uploadingDoc} onChange={e => uploadDoc(e, cat.key)} />
                            </label>
                          </div>
                          {catDocs.length === 0 ? (
                  ...
                  <div>
                    {!isDriverRole(selected) ? (
                      <div style={{ textAlign:'center', color:'#8A95A3', fontSize:'11px', padding:'24px', background:'#F8F9FB', borderRadius:'8px' }}>
                        Ce membre n'est pas conducteur.
                      </div>
                    ) : (
                      <>
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'8px' }}>
                        ...
                        {driverCircuits.length === 0 ? (
                          <div style={{ textAlign:'center', color:'#8A95A3', fontSize:'11px', padding:'12px', background:'#F8F9FB', borderRadius:'8px', marginBottom:'8px' }}>Aucun circuit habituel assigné</div>
                        ) : (
                          <div style={{ display:'flex', flexDirection:'column', gap:'6px', marginBottom:'10px' }}>
                            {driverCircuits.map((dc: any) => (
                              <div key={dc.id} style={{ background:'#E8EAF0', borderRadius:'6px', padding:'8px 10px', display:'flex', alignItems:'center', gap:'8px' }}>
                                ...
                                {editingCircuits && <button onClick={() => removeDriverCircuit(dc.id)} style={{ background:'none', border:'none', color:'#C62828', cursor:'pointer', fontSize:'14px' }}>🗑</button>}
                              </div>
                            ))}
                          </div>
                        )}
                        {editingCircuits && circuits.length > 0 && <CircuitAssigner circuits={circuits} driverCircuits={driverCircuits} onAdd={addDriverCircuit} saving={savingCircuit} />}
                      </>
                    )}
                  </div>
                )}
                ...
                {selected && !showForm && tab === 'absences' && (
                  <div>
                    <div style={{ fontSize:'11px', fontWeight:'700', color:'#1A2130', marginBottom:'10px' }}>🧳 Absences — {selected.name}</div>
                    {absences.length === 0 ? (
                      <div style={{ textAlign:'center', color:'#8A95A3', fontSize:'11px', padding:'24px', background:'#F8F9FB', borderRadius:'8px' }}>Aucune demande</div>
                    ) : absences.map((a: any) => {
                      const st = STATUT_ABS[a.statut] || STATUT_ABS.pending
                      return (
                        <div key={a.id} style={{ background:'white', border:'1px solid #E2E6EA', borderRadius:'8px', padding:'10px 12px', marginBottom:'8px' }}>
      ...
      <div style={{ background:'#253044', padding:'6px 16px', display:'flex', gap:'20px', flexShrink:0 }}>
        {[
          ...
          <div key={l as string}>
            <div style={{ fontSize:'14px', fontWeight:'700', color: (v > 0 && (l === 'Alertes certifications' || l === 'Absences en attente')) ? '#EF9A9A' : 'white' }}>{v}</div>
